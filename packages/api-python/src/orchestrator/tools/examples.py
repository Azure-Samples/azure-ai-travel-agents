...
async def example_2_specific_servers():
    ...
    travel_tools = await tool_registry.get_all_tools(
        servers=["itinerary-planning", "destination-recommendation"]
    )
...
async def example_6_agent_with_tools():
    ...
    # Load MCP tools
    mcp_tools = await tool_registry.get_all_tools(
        servers=["itinerary-planning", "customer-query", "echo-ping"]
    )
